...
const AppointmentSlotForm: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl mx-auto p-6 bg-white shadow rounded space-y-6">
      ...
      <div>
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="flex items-center space-x-4 mb-3">
            <input
              type="time"
              {...register(`slots.${index}.startTime`, { required: 'Start time required' })}
              className="border border-gray-300 rounded p-2"
            />
            <span>to</span>
            <input
              type="time"
              {...register(`slots.${index}.endTime`, { required: 'End time required' })}
              className="border border-gray-300 rounded p-2"
            />
            <button
              type="button"
